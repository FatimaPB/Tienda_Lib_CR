...
export class IncidenciasComponent implements OnInit {
  ...
  obtenerActividades(): void {
    this.http.get<{ actividades: Actividad[] }>('https://api-libreria.vercel.app/api/actividad')
      .subscribe(
        (data) => {
...
cargarUsuarios(): void {
  this.http.get<Usuario[]>('https://api-libreria.vercel.app/api/usuarios') // Cambia la URL según tu API
    .subscribe((data) => {
  ...
  obtenerUsuariosBloqueados(): void {
    this.http.get<Usuario[]>('https://api-libreria.vercel.app/api/usuarios-bloqueados?periodo=dia') // Cambia la URL según tu API
      .subscribe((data) => {
    ...
    obtenerUsuariosBloqueadosmes(): void {
      this.http.get<Usuario[]>('https://api-libreria.vercel.app/api/usuarios-bloqueados?periodo=mes') // Cambia la URL según tu API
        .subscribe((data) => {
          this.usuariosBloqueados = data;
        }, (error) => {
          console.error('Error al cargar usuarios bloqueados:', error);
        ...
        obtenerUsuariosBloqueadossemana(): void {
          this.http.get<Usuario[]>('https://api-libreria.vercel.app/api/usuarios-bloqueados?periodo=semana') // Cambia la URL según tu API
            .subscribe((data) => {
              this.usuariosBloqueados = data;
            }, (error) => {
              console.error('Error al cargar usuarios bloqueados:', error);
...
bloquearUsuarioDesdeLista(usuario: Usuario): void {
  ...
  this.http.put(`https://api-libreria.vercel.app/api/usuarios/bloquear/${userId}`, {})
    .subscribe((response) => {
...
async guardarConfiguracionBloqueo() {
  ...
  try {
      const response = await fetch('https://api-libreria.vercel.app/api/limite-intentos', {
          method: 'PUT',
          headers: {
