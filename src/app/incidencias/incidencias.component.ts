...
cargarUsuarios(): void {
  this.http.get<Usuario[]>('http://localhost:3000/api/usuarios') // Cambia la URL según tu API
    .subscribe((data) => {
  ...
  obtenerUsuariosBloqueados(): void {
    this.http.get<Usuario[]>('http://localhost:3000/api/usuarios-bloqueados?periodo=dia') // Cambia la URL según tu API
      .subscribe((data) => {
    ...
    obtenerUsuariosBloqueadosmes(): void {
      this.http.get<Usuario[]>('http://localhost:3000/api/usuarios-bloqueados?periodo=mes') // Cambia la URL según tu API
        .subscribe((data) => {
          this.usuariosBloqueados = data;
        }, (error) => {
          console.error('Error al cargar usuarios bloqueados:', error);
        ...
        obtenerUsuariosBloqueadossemana(): void {
          this.http.get<Usuario[]>('http://localhost:3000/api/usuarios-bloqueados?periodo=semana') // Cambia la URL según tu API
            .subscribe((data) => {
              this.usuariosBloqueados = data;
            }, (error) => {
              console.error('Error al cargar usuarios bloqueados:', error);
...
bloquearUsuarioDesdeLista(usuario: Usuario): void {
  ...
  this.http.put(`http://localhost:3000/api/usuarios/bloquear/${userId}`, {})
    .subscribe((response) => {
