...
export class PoliticasComponent {
  ...
  obtenerDocumentos() {
    this.http.get('http://localhost:3000/api/documentos/historial').subscribe((data: any) => {
      this.documentos = data;
    });
  }

  obtenerDocumentoVigente() {
    this.http.get('http://localhost:3000/api/documentos/vigente').subscribe({
      next: (data: any) => {
  ...
  crearDocumento() {
    ...
    this.http.post('http://localhost:3000/api/documentos', this.documento).subscribe({
      next: () => {
        this.obtenerDocumentos();
  ...
  actualizarDocumento() {
    if (this.editingId) {
      this.http.post(`http://localhost:3000/api/documentos/${this.editingId}/version`, this.documento).subscribe(() => {
        this.obtenerDocumentos();
        this.limpiarFormulario();
        this.obtenerDocumentoVigente();
      });
  ...
  eliminarDocumento(id: string) {
    this.http.delete(`http://localhost:3000/api/documentos/${id}`).subscribe(() => {
      this.obtenerDocumentos();
      this.obtenerDocumentoVigente();
    });
  }
