...
export class TerminosComponent {
  ...
  obtenerDocumentos() {
    this.http.get('http://localhost:3000/api/terminos/historial').subscribe((data: any) => {
      this.documentos = data;
    });
  }

  obtenerDocumentoVigente() {
    this.http.get('http://localhost:3000/api/terminos/vigente').subscribe({
      next: (data: any) => {
  ...
  crearDocumento() {
    ...
    this.http.post('http://localhost:3000/api/terminos', this.documento).subscribe({
      next: () => {
        this.obtenerDocumentos();
  ...
  actualizarDocumento() {
    if (this.editingId) {
      this.http.post(`http://localhost:3000/api/terminos/${this.editingId}/version`, this.documento).subscribe(() => {
        this.obtenerDocumentos();
        this.limpiarFormulario();
        this.obtenerDocumentoVigente();
      });
  ...
  eliminarDocumento(id: string) {
    this.http.delete(`http://localhost:3000/api/terminos/${id}`).subscribe(() => {
      this.obtenerDocumentos();
      this.obtenerDocumentoVigente();
    });
  }
